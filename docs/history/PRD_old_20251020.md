...
### 디자인 시스템 (packages/pickly_design_system/)

**배치 기준**:
- **3곳 이상**에서 사용되는 공통 컴포넌트만 배치
- 비즈니스 로직 없음 (순수 UI 컴포넌트)
- 재사용성과 범용성이 높은 컴포넌트

**예시**:
- `SelectionChip`: 선택 가능한 칩 버튼 (지역, 카테고리 등)
- `SelectionCheckmark`: 선택 상태 표시 체크마크
- `PicklyButton`: 기본 버튼 컴포넌트
- `ProgressBar`: 진행률 표시기
- `SelectionListItem`: 리스트형 선택 항목
...
### Feature 위젯 (apps/pickly_mobile/lib/features/)
...
**예시**:
- `OnboardingHeader`: 온보딩 전용 헤더 (진행률 + 뒤로가기)
- `PolicyCard`: 정책 피드 전용 카드
- `FilterBottomSheet`: 검색 필터 바텀시트

**특징**:
- Feature Context에 종속적
- Provider/State와 강결합 가능
- 화면별 비즈니스 규칙 구현
- 디자인 시스템 컴포넌트 조합 우선
...
import 'package:pickly_mobile/features/policy/widgets/policy_card.dart';
```

### 컴포넌트화 원칙

1. **3회 규칙**: 같은 패턴이 3곳 이상 등장하면 컴포넌트화
2. **범용성 우선**: 범용적이면 Design System으로
3. **비즈니스 로직 분리**: UI 컴포넌트는 로직 없이
4. **Props 기반 제어**: 상태는 외부에서 주입
5. **테스트 가능성**: 모든 컴포넌트는 위젯 테스트 포함
...
컴포넌트 구조에 대한 자세한 내용은 다음 문서를 참고하세요:
- [Component Structure Guide](./architecture/component-structure-guide.md)
- [Project Structure Guide](./architecture/project-structure.md)
- [Import Conventions](./architecture/import-conventions.md)
